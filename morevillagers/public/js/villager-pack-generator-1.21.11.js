// Villager Pack Generator for Minecraft 1.21.11
// https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js

class VillagerPackGenerator_1_21_11 extends VillagerPackGeneratorBase {
    async generatePack() {
        const zip = new JSZip();
        ...
        try {
            ...
            // Generate workstations (1.21.11 format)
            workstations.forEach(ws => {
                if (ws.name) {
                    ...
                    // Block model (simplified texture mapping for 1.21.11)
                    const blockModel = {
                        parent: "minecraft:block/cube",
                        textures: {
                            east: `${wsNamespace}:block/${ws.name}_side`,
                            particle: `${wsNamespace}:block/${ws.name}_front`,
                            west: `${wsNamespace}:block/${ws.name}_front`,
                    ...
                    // Item model (1.21.11 format - NEW!)
                    const itemModel = {
                        model: {
                            type: "minecraft:model",
                            model: `${wsNamespace}:item/${ws.name}`
                        }
                    };
                    zip.file(`assets/${wsNamespace}/items/${ws.name}.json`, JSON.stringify(itemModel, null, 2));

                    // Item model reference
                    const itemModelRef = {
                        parent: `${wsNamespace}:block/${ws.name}`
                    };
                    zip.file(`assets/${wsNamespace}/models/item/${ws.name}.json`, JSON.stringify(itemModelRef, null, 2));

                    // Block textures
                    const textures = this.packData.textures.workstations[ws.name];
                    if (textures) {
                        // Map front to west/front, back to east/side, left to north, right to south
                        if (textures.textureFront) {
                            const base64Data = textures.textureFront.split(',')[1];
                            zip.file(`assets/${wsNamespace}/textures/block/${ws.name}_front.png`, base64Data, { base64: true });
                        }
                        if (textures.textureBack || textures.textureLeft || textures.textureRight) {
                            // Use back as side texture, or left/right if back not provided
                            const sideTexture = textures.textureBack || textures.textureLeft || textures.textureRight;
                            const base64Data = sideTexture.split(',')[1];
                            zip.file(`assets/${wsNamespace}/textures/block/${ws.name}_side.png`, base64Data, { base64: true });
                        }
                        if (textures.textureTop) {
                            const base64Data = textures.textureTop.split(',')[1];
                            zip.file(`assets/${wsNamespace}/textures/block/${ws.name}_top.png`, base64Data, { base64: true });
                        }
                        if (textures.textureBottom) {
                            const base64Data = textures.textureBottom.split(',')[1];
                            zip.file(`assets/${wsNamespace}/textures/block/${ws.name}_bottom.png`, base64Data, { base64: true });
                        }
                    }
            ...
            // Generate biome mappings (still in villagers folder)
            ...
            // ALSO generate NeoForge data map for biome types (1.21.11 addition)
            if (biomeMappings.length > 0) {
                const biomeDataMap = { values: {} };
                biomeMappings.forEach(mapping => {
                    if (mapping.biomesJson && mapping.biomesJson.biomes) {
                        Object.entries(mapping.biomesJson.biomes).forEach(([biome, villagerType]) => {
                            biomeDataMap.values[biome] = {
                                villager_type: villagerType.includes(':') ? villagerType : `${namespace}:${villagerType}`
                            };
                        });
                    }
                });
                if (Object.keys(biomeDataMap.values).length > 0) {
                    zip.file('data/neoforge/data_maps/worldgen/biome/villager_types.json', JSON.stringify(biomeDataMap, null, 2));
                }
            }

            // Generate loot tables with random_sequence (1.21.11 format)
            lootTables.forEach(loot => {
                if (loot.name && loot.lootJson) {
                    // Add random_sequence if not present
                    if (!loot.lootJson.random_sequence) {
                        loot.lootJson.random_sequence = `${namespace}:gameplay/hero_of_the_village/${loot.name}`;
                    }
            ...
            workstations.forEach(ws => {
                if (ws.name) {
                    ...
                    langData[`item.${wsNamespace}.${ws.name}`] = this.toTitleCase(ws.name);
                }
            });
            structureTags.forEach(tag => {
            ...
            langData[`itemGroup.${namespace}.tab`] = this.packData.metadata.displayName || this.toTitleCase(namespace);
            ...
            this.downloadBlob(content, `${packName}_1.21.11.zip`);

            alert('Pack generated successfully for Minecraft 1.21.11!');
        } catch (error) {
